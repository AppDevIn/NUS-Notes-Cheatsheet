...
public class GameTree {
    ...
    int findValue() {
        ...
        return this.findValue(root, Player.ONE);
    }


    int findValue(TreeNode node, Player player) {
        if (node.leaf)
            return node.value;

        int value = (player == Player.ONE) ? Integer.MIN_VALUE : Integer.MAX_VALUE;

        for (int i = 0; i < node.numChildren; i++) {
            int childValue = findValue(node.children[i], other(player));

            value = (player == Player.ONE) ?
                    Math.max(childValue, value) :
                    Math.min(childValue, value);
        }
